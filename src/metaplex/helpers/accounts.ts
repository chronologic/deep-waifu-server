  ...
  return {
    ...
    txId: await anchorProgram.rpc.initializeConfig(
      ...
      },
      {
        ...
        instructions: [
          await createConfigAccount(anchorProgram, configData, payerWallet.publicKey, configAccount.publicKey),
        ],
      }
    ),
  };
...
export const getCandyMachineAddress = async (config: anchor.web3.PublicKey, uuid: string) => {
...
export const getConfig = async (authority: anchor.web3.PublicKey, uuid: string) => {
  return await anchor.web3.PublicKey.findProgramAddress(
